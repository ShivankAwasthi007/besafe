      ...
      <div class="col-md-9 col-xl-7">
        <div class="card-header px-0 mt-2 bg-transparent clearfix">
          ...
          <div class="card-header-actions mr-1">
            ...
            <a
              v-if="plan.is_pay_as_you_go != 1 && plan.is_free != 1" 
        ...
        <div class="card-body px-0">
          <div class="row" v-if="!loading">
            <div class="form-group col-md-9">
              ...
              v-model="plan.name" placeholder="Enter plane name" :disabled="plan.price ==0 || plan.is_pay_as_you_go == 1">
            ...
            <div class="col-md-12">
              <div class="form-group">
                <label v-if="plan.is_pay_as_you_go == 1">Price per {{billing_cycle}} (per child)</label>
                <label v-else>Price per {{billing_cycle}}</label>
                <input type="text"
                class="form-control" :class="{'is-invalid': errors.price}" 
                v-model="plan.price" 
                :disabled="plan.is_free == 1"
                :placeholder=currency>
                <div class="invalid-feedback" v-if="errors.price">{{errors.price[0]}}</div>
              </div>
              <div v-if="plan.is_pay_as_you_go != 1" class="form-group">
              ...
              <div v-if="plan.is_pay_as_you_go != 1" class="form-group">
                <label>Maximum number of seats (children)</label>
                <input type="text" class="form-control"
                       :class="{'is-invalid': errors.allowed_children}" v-model="plan.allowed_children">
                <div class="invalid-feedback" v-if="errors.allowed_children">{{errors.allowed_children[0]}}</div>
                <small>Enter -1 for unlimited</small>
              </div>
            </div>
          </div>
    ...
    update () {
      if (!this.submiting) {
        ...
        .catch(error => {
          ...
          swal("Error", error.response.data.errors.Plan[0], "error")
        })
      }
    },
    destroy() {
          ...
          if (willDelete) {
            axios
              ...
              .catch(error => {
                this.submitingDestroy = false;
                this.errors = error.response.data.errors;
                if(error.response.data.errors.Plan[0])
                {
                  swal("Error", error.response.data.errors.Plan[0], "error")
                }
              });
